...
class ReservaRepository
{
    ...
    public function obtenerPorFechas($fecha_inicio, $fecha_fin)
    {
        ...
        ORDER BY u.nombre, m.num_mesa;");

            $stmt->execute(['fecha_fin' => $fecha_fin->format('Y-m-d H:i:s'), 'fecha_inicio' => $fecha_inicio->format('Y-m-d H:i:s')]);

            return  $stmt->fetchAll(PDO::FETCH_ASSOC); //recupera todas las filas en un array, por defecto se utiliza PDO::FETCH_BOTH
        } catch (Exception $e) {
